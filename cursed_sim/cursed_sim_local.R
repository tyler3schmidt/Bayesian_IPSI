...
compute_simulation <- function(n, I, J) {
  ...
  ipsi_reg_bayes_psi_hat <- colMeans(ipsi_reg_psi_matrix$psi_delta_matrix)
  ipsi_trans_bayes_psi_hat <- colMeans(ipsi_trans_psi_matrix$psi_delta_matrix)
  
  ipsi_reg_bayes_efficient_psi_hat <- colMeans(ipsi_reg_psi_matrix$efficient_influence_matrix)
  ipsi_trans_bayes_efficient_psi_hat <- colMeans(ipsi_trans_psi_matrix$efficient_influence_matrix)
  
  ipsi_reg_bayes_coverage <- bayesian_coverage(est_matrix = ipsi_reg_psi_matrix$psi_delta_matrix, true_vec = ipsi_psi_true)
  ipsi_trans_bayes_coverage <- bayesian_coverage(est_matrix = ipsi_trans_psi_matrix$psi_delta_matrix, true_vec = ipsi_psi_true)
  
  ipsi_reg_bayes_efficient_coverage <- bayesian_coverage(est_matrix = ipsi_reg_psi_matrix$efficient_influence_matrix, true_vec = ipsi_psi_true)  
  ipsi_trans_bayes_efficient_coverage <- bayesian_coverage(est_matrix = ipsi_trans_psi_matrix$efficient_influence_matrix, true_vec = ipsi_psi_true)
  ...
  return(list(
    ...
    # bayesian
    ipsi_reg_bayes_psi_hat = ipsi_reg_bayes_psi_hat,
    ipsi_reg_bayes_efficient_psi_hat = ipsi_reg_bayes_efficient_psi_hat,
    ipsi_reg_bayes_coverage = ipsi_reg_bayes_coverage,
    ipsi_reg_bayes_efficient_coverage = ipsi_reg_bayes_efficient_coverage,
    ipsi_trans_bayes_psi_hat = ipsi_trans_bayes_psi_hat,
    ipsi_trans_bayes_efficient_psi_hat = ipsi_trans_bayes_efficient_psi_hat,
    ipsi_trans_bayes_coverage = ipsi_trans_bayes_coverage,
    ipsi_trans_bayes_efficient_coverage = ipsi_trans_bayes_efficient_coverage
  ))
}
...
all_sims <- vector("list", J)
for (j in 1:J) {
  all_sims[[j]] <- compute_simulation(n = n, I = I, J = j)
  cat("Finished sim", j, "of", J, "\n")
  flush.console() 
}

local_summary <- function(all_sims, ipsi_true_psi, n, I, J) {
  ...
  ipsi_reg_bayes_psi_hat <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_efficient_psi_hat <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_uniform_coverage <- numeric(J)
  ipsi_reg_bayes_pointwise_coverage <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_uniform_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_pointwise_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_efficient_uniform_coverage <- numeric(J)
  ipsi_reg_bayes_efficient_pointwise_coverage <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_efficient_uniform_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_reg_bayes_efficient_pointwise_length <- matrix(NA, nrow = J, ncol = I)
  
  ipsi_trans_bayes_psi_hat <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_efficient_psi_hat <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_uniform_coverage <- numeric(J)
  ipsi_trans_bayes_pointwise_coverage <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_uniform_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_pointwise_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_efficient_uniform_coverage <- numeric(J)
  ipsi_trans_bayes_efficient_pointwise_coverage <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_efficient_uniform_length <- matrix(NA, nrow = J, ncol = I)
  ipsi_trans_bayes_efficient_pointwise_length <- matrix(NA, nrow = J, ncol = I)
  ...
  for (j in 1:J) {
    ...
    ipsi_reg_bayes_psi_hat[j,] <- all_sims[[j]]$ipsi_reg_bayes_psi_hat
    ipsi_reg_bayes_efficient_psi_hat[j,] <- all_sims[[j]]$ipsi_reg_bayes_efficient_psi_hat
    ipsi_reg_bayes_uniform_coverage[j] <- all_sims[[j]]$ipsi_reg_bayes_coverage$uniform_coverage
    ipsi_reg_bayes_pointwise_coverage[j, ] <- all_sims[[j]]$ipsi_reg_bayes_coverage$pointwise_coverage
    ipsi_reg_bayes_uniform_length[j,] <- all_sims[[j]]$ipsi_reg_bayes_coverage$uniform_length
    ipsi_reg_bayes_pointwise_length[j, ] <- all_sims[[j]]$ipsi_reg_bayes_coverage$pointwise_length
    ipsi_reg_bayes_efficient_uniform_coverage[j] <- all_sims[[j]]$ipsi_reg_bayes_efficient_coverage$uniform_coverage
    ipsi_reg_bayes_efficient_pointwise_coverage[j, ] <- all_sims[[j]]$ipsi_reg_bayes_efficient_coverage$pointwise_coverage
    ipsi_reg_bayes_efficient_uniform_length[j,] <- all_sims[[j]]$ipsi_reg_bayes_efficient_coverage$uniform_length
    ipsi_reg_bayes_efficient_pointwise_length[j, ] <- all_sims[[j]]$ipsi_reg_bayes_efficient_coverage$pointwise_length
    
    ipsi_trans_bayes_psi_hat[j,] <- all_sims[[j]]$ipsi_trans_bayes_psi_hat
    ipsi_trans_bayes_efficient_psi_hat[j,] <- all_sims[[j]]$ipsi_trans_bayes_efficient_psi_hat
    ipsi_trans_bayes_uniform_coverage[j] <- all_sims[[j]]$ipsi_trans_bayes_coverage$uniform_coverage
    ipsi_trans_bayes_pointwise_coverage[j, ] <- all_sims[[j]]$ipsi_trans_bayes_coverage$pointwise_coverage
    ipsi_trans_bayes_uniform_length[j, ] <- all_sims[[j]]$ipsi_trans_bayes_coverage$uniform_length
    ipsi_trans_bayes_pointwise_length[j, ] <- all_sims[[j]]$ipsi_trans_bayes_coverage$pointwise_length
    ipsi_trans_bayes_efficient_uniform_coverage[j] <- all_sims[[j]]$ipsi_trans_bayes_efficient_coverage$uniform_coverage
    ipsi_trans_bayes_efficient_pointwise_coverage[j, ] <- all_sims[[j]]$ipsi_trans_bayes_efficient_coverage$pointwise_coverage
    ipsi_trans_bayes_efficient_uniform_length[j, ] <- all_sims[[j]]$ipsi_trans_bayes_efficient_coverage$uniform_length
    ipsi_trans_bayes_efficient_pointwise_length[j, ] <- all_sims[[j]]$ipsi_trans_bayes_efficient_coverage$pointwise_length
  }
  
  # bias / rmse (freq + bayes)
  ipsi_reg_bias <- bias_est(ipsi_reg_psi_hat, ipsi_true_psi, I, J)
  ipsi_reg_efficient_bias <- bias_est(ipsi_reg_efficient_psi_hat, ipsi_true_psi, I, J)
  ipsi_reg_rmse <- RMSE_est(ipsi_reg_psi_hat, ipsi_true_psi, I, J, n)
  ipsi_reg_efficient_rmse <- RMSE_est(ipsi_reg_efficient_psi_hat, ipsi_true_psi, I, J, n)
  
  ipsi_trans_bias <- bias_est(ipsi_trans_psi_hat, ipsi_true_psi, I, J)
  ipsi_trans_efficient_bias <- bias_est(ipsi_trans_efficient_psi_hat, ipsi_true_psi, I, J)
  ipsi_trans_rmse <- RMSE_est(ipsi_trans_psi_hat, ipsi_true_psi, I, J, n)
  ipsi_trans_efficient_rmse <- RMSE_est(ipsi_trans_efficient_psi_hat, ipsi_true_psi, I, J, n)
  
  ipsi_reg_bayes_bias <- bias_est(ipsi_reg_bayes_psi_hat, ipsi_true_psi, I, J)
  ipsi_reg_bayes_efficient_bias <- bias_est(ipsi_reg_bayes_efficient_psi_hat, ipsi_true_psi, I, J)
  ipsi_reg_bayes_rmse <- RMSE_est(ipsi_reg_bayes_psi_hat, ipsi_true_psi, I, J, n)
  ipsi_reg_bayes_efficient_rmse <- RMSE_est(ipsi_reg_bayes_efficient_psi_hat, ipsi_true_psi, I, J, n)
  
  ipsi_trans_bayes_bias <- bias_est(ipsi_trans_bayes_psi_hat, ipsi_true_psi, I, J)
  ipsi_trans_bayes_efficient_bias <- bias_est(ipsi_trans_bayes_efficient_psi_hat, ipsi_true_psi, I, J)
  ipsi_trans_bayes_rmse <- RMSE_est(ipsi_trans_bayes_psi_hat, ipsi_true_psi, I, J, n)
  ipsi_trans_bayes_efficient_rmse <- RMSE_est(ipsi_trans_bayes_efficient_psi_hat, ipsi_true_psi, I, J, n)
  ...
  return(list(
    ...
    ipsi_reg_bayes_uniform_coverage = ipsi_reg_bayes_uniform_coverage,
    ipsi_reg_bayes_pointwise_coverage = ipsi_reg_bayes_pointwise_coverage,
    ipsi_reg_bayes_uniform_length = ipsi_reg_bayes_uniform_length,
    ipsi_reg_bayes_pointwise_length = ipsi_reg_bayes_pointwise_length,
    ipsi_reg_bayes_efficient_uniform_coverage = ipsi_reg_bayes_efficient_uniform_coverage,
    ipsi_reg_bayes_efficient_pointwise_coverage = ipsi_reg_bayes_efficient_pointwise_coverage,
    ipsi_reg_bayes_efficient_uniform_length = ipsi_reg_bayes_efficient_uniform_length,
    ipsi_reg_bayes_efficient_pointwise_length = ipsi_reg_bayes_efficient_pointwise_length,
    ipsi_reg_bayes_bias = ipsi_reg_bayes_bias,
    ipsi_reg_bayes_efficient_bias = ipsi_reg_bayes_efficient_bias,
    ipsi_reg_bayes_rmse = ipsi_reg_bayes_rmse,
    ipsi_reg_bayes_efficient_rmse = ipsi_reg_bayes_efficient_rmse,
    
    ipsi_trans_bayes_uniform_coverage = ipsi_trans_bayes_uniform_coverage,
    ipsi_trans_bayes_pointwise_coverage = ipsi_trans_bayes_pointwise_coverage,
    ipsi_trans_bayes_uniform_length = ipsi_trans_bayes_uniform_length,
    ipsi_trans_bayes_pointwise_length = ipsi_trans_bayes_pointwise_length,
    ipsi_trans_bayes_efficient_uniform_coverage = ipsi_trans_bayes_efficient_uniform_coverage,
    ipsi_trans_bayes_efficient_pointwise_coverage = ipsi_trans_bayes_efficient_pointwise_coverage,
    ipsi_trans_bayes_efficient_uniform_length = ipsi_trans_bayes_efficient_uniform_length,
    ipsi_trans_bayes_efficient_pointwise_length = ipsi_trans_bayes_efficient_pointwise_length,
    ipsi_trans_bayes_bias = ipsi_trans_bayes_bias,
    ipsi_trans_bayes_efficient_bias = ipsi_trans_bayes_efficient_bias,
    ipsi_trans_bayes_rmse = ipsi_trans_bayes_rmse,
    ipsi_trans_bayes_efficient_rmse = ipsi_trans_bayes_efficient_rmse
  ))
}

data <- local_summary(all_sims, ipsi_true_psi, n, I, J)
